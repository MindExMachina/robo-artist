...
function drawRectStrokes(height, width)
{

    // rectPath = new Path2D();
    // rectPath.rect(height,height,width,width);
    // rectStroke = new stroke(rectPath);
    // let rect = new SketchRNNStroke(tolerance);
    //         rect.addStroke(rectStroke);

    drawingDoodle.down(0, 0);
    drawingDoodle.add(0, width);
    drawingDoodle.add(height, width);
    drawingDoodle.add(height, 0);
    drawingDoodle.up(0, 0);
    completedDoodles.push(drawingDoodle);
    drawingDoodle = new SketchRNNDoodle(tolerance);
...
//     var polylineLength = pointsString.length;
//     if (polylineLength > 20){//cull small polylines that create noise, only draw bigger ones
...
function drawPolylineStrokes(polylines){
    for (var i = 0; i < polylines.length; i++) {
              console.log('polyline #' + i + ' length: ' + polylines[i].length);
              drawPolylineStroke(polylines[i]);
}
}
